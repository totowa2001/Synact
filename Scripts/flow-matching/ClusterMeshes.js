// 최신화 260525
// 최신화내용: 2.5D 전환 — CircleGeometry → SphereGeometry, MeshLambertMaterial로 입체감
// 스크립트 이름: ClusterMeshes.js
// 스크립트 기능: Flow Matching의 데이터 분포 p₀→p₁ 수렴을 구 메시로 시각화.
//               globalT=0: 가우시안 분포(중심 밀집), globalT=1: CLUSTERS 좌표로 수렴.
//               smoothstep 이징, 수렴할수록 구 크기 성장. SphereGeometry + Lambert로 2.5D 입체감.
//               색상은 CLUSTER_COLORS_HEX 팔레트 순환 (#0D0D3E 계열 다크 인디고).
//
// [ 핵심 구조 ]
//   _mesh — THREE.InstancedMesh (SphereGeometry(0.36, 8, 6), particleCount개)
//   _p0   — Float32Array (TOTAL*2) 가우시안 시작 위치 (XY)
//   _p1   — Float32Array (TOTAL*2) 클러스터 수렴 위치 (XY)
//
...
var ClusterMeshes = (function () {
  ...
  // 함수 이름: init
  // 함수 기능: 가우시안/클러스터 XY 위치 사전 계산, SphereGeometry InstancedMesh 초기화,
  //           색상을 CLUSTER_COLORS_HEX 팔레트로 1회 설정
  // 입력 파라미터: scene (THREE.Scene)
  // 리턴 타입: void
  function init(scene) {
    _scene = scene;
    _TOTAL = PARAMS.particleCount;
    _p0 = new Float32Array(_TOTAL * 2);
    _p1 = new Float32Array(_TOTAL * 2);

    var sigma = PARAMS.gridHalf * 0.50;

    for (var i = 0; i < _TOTAL; i++) {
      // 가우시안 분포 시작 위치 (XY)
      _p0[i*2]   = _gauss(i * 13 +  0) * sigma;
      _p0[i*2+1] = _gauss(i * 13 +  2) * sigma;

      // 클러스터 수렴 위치 (XY)
      var ci = i % CLUSTERS.length;
      var cl = CLUSTERS[ci];
      _p1[i*2]   = cl.x + (_rnd(i * 17 + 1) * 2 - 1) * cl.r * 0.8;
      _p1[i*2+1] = cl.y + (_rnd(i * 17 + 3) * 2 - 1) * cl.r * 0.8;
    }

    // 구 메시: Lambert 재질 → 방향광 음영으로 2.5D 입체감
    var geo = new THREE.SphereGeometry(0.36, 8, 6);
  ...
  // 함수 이름: update
  // 함수 기능: globalT의 smoothstep으로 _p0→_p1 XY 위치 보간, 수렴할수록 구 크기 성장
  ...
  function update(globalT) {
    ...
    for (var i = 0; i < _TOTAL; i++) {
      var x = _p0[i*2]   + st * (_p1[i*2]   - _p0[i*2]);
      var y = _p0[i*2+1] + st * (_p1[i*2+1] - _p0[i*2+1]);

      _dummy.position.set(x, y, -0.2);
